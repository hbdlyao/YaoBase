...
void CxbRw::doLoad_xbMonitor()
{
	doLoad_xbMonitor_Root();
	...
	{
		vDev = dynamic_cast<CxbDevMonitor *>(vPair.second);

		//
		doLoad_xbMonitor_Tree(vDev,1);


	}//for each

	//doLoad_xbMonitor_Tree();

}

//
void CxbRw::doLoad_xbMonitor_Root()
{
	CDeviceTBL * vTBL;
	CxbDevMonitor * vDev;
	...
	vSQL = "select * from xbMonitor";
	vOk = RwAdo->OpenSQL(vSQL);

	if (vOk)
	{
		cout << "Load---xbMonitor---" << endl;

		while (!RwAdo->IsEOF())
		{
			vTBL = pHvdc->DeviceTBL(xb_Monitor);
			vDev = dynamic_cast<CxbDevMonitor *>(vTBL->NewDevice(xb_Monitor));
...
//
void CxbRw::doLoad_xbMonitor_Tree(CxbDevMonitor * vParent,int vIndex)
{
	CDeviceTBL * vTBL;
	CxbDevBranch * vDev;
	...
	//
	vSQL = "select * from xbMonitor_tree where RootDeviceName= ";
	vSQL = vSQL + " '";
...
void CxbRw::doSave_xbMonitor()
{
	doSave_xbMonitor_Root();
...
//
void CxbRw::doSave_xbMonitor_Root()
{
	CDeviceTBL * vTBL;
	CxbDevMonitor * vDev;
	...
	if (vOk)
	{
		cout << "Save---xbMonitor---" << endl;

		vTBL = pHvdc->DeviceTBL(xb_Monitor);
		for (i = 0; i<vTBL->GetItemCount(); i++)
		{

			vDev = dynamic_cast<CxbDevMonitor *>(vTBL->Device(i));
			//
			RwAdo->Record_AddNew();
...
//
void CxbRw::doSave_xbMonitor_Tree(CxbDevMonitor * vParent, int vIndex)
{
	CDeviceTBL * vTBL;
	CxbDevBranch * vDev;
	...
	vSQL = "delete * from xbMonitor_tree where RootDeviceName= ";
	vSQL = vSQL + " '";
	vSQL = vSQL + vParent->GetDeviceName();
	vSQL = vSQL + "' ";
	vSQL = vSQL + "and TreeLevel= " + to_string(vIndex);
	...
	vSQL = "select * from xbMonitor_tree ";
	if (vOk)
		vOk = vOk && (RwAdo->OpenSQL(vSQL));

	if (vOk)
	{
		cout << "Save---xbMonitor_tree---" << endl;
...
void CxbRw::doLoad_Order(CxbOrder* vOrder)
{
	string vSQL, vStr;
	_variant_t vValue;
	bool vOk;
...
void CxbRw::doSave_Order(CxbOrder* vOrder)
{

}

void CxbRw::doLoad_Result(CxbResult * vRes)
{
...
void CxbRw::doSave_Result(CxbResult * vRes)
{
...
void CxbRw::doLoad_Data(){

}


void CxbRw::doSave_Data(){
