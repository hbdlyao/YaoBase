...
void CxbDevDCF::InitDev(int vDCFType, int vTimes)
{
	...
	case DCF_Single:
		...
		OneTuned.ConstDevYg = new double[hMax() * 3];
		OneTuned.ConstDevYb = new double[hMax() * 3];
		OneTuned.RandDevYg = new double[hMax() * vTimes];
		OneTuned.RandDevYb = new double[hMax() * vTimes];
		SetOneConstDev();
	...
	case DCF_Double:
		...
		TwoTuned.ConstDevYg = new double[hMax() * 3];
		TwoTuned.ConstDevYb = new double[hMax() * 3];
		TwoTuned.RandDevYg = new double[hMax() * vTimes];
		TwoTuned.RandDevYb = new double[hMax() * vTimes];
		SetTwoConstDev();
	...
	case DCF_Triple:
		...
		ThreeTuned.ConstDevYg = new double[hMax() * 3];
		ThreeTuned.ConstDevYb = new double[hMax() * 3];
		ThreeTuned.RandDevYg = new double[hMax() * vTimes];
		ThreeTuned.RandDevYb = new double[hMax() * vTimes];
		SetThreeConstDev();
	...
	for (int vCount = 0; vCount < 3; vCount++)
		for (int vhOrder = 1; vhOrder <= hMax(); vhOrder++)
		{
			...
			case DCF_Single:
				vw = PI * 2 * (Frequency)* vhOrder;
				vY = CComplex((OneTuned.R1 + OneTuned.ConstDev->ddR1), vw *(OneTuned.L1 + OneTuned.ConstDev->ddL1) - 1.0 / vw / (OneTuned.C1 + OneTuned.ConstDev->ddC1)).inverse();
				OneTuned.ConstDevYg[vhOrder - 1 + hMax() * vCount] = vY.real();
				OneTuned.ConstDevYb[vhOrder - 1 + hMax() * vCount] = vY.image();
				break;
			case DCF_Double:
				vw = PI * 2 * (Frequency)* vhOrder;
				if (TwoTuned.R1 == 0)
				{
					...
					vZ = vY.inverse() + CComplex(0, vw * (TwoTuned.L1 + TwoTuned.ConstDev->ddL1) - 1.0 / vw / (TwoTuned.C1 + TwoTuned.ConstDev->ddC1));
					vY = vZ.inverse();
					TwoTuned.ConstDevYg[vhOrder - 1 + hMax() * vCount] = vY.real();
					TwoTuned.ConstDevYb[vhOrder - 1 + hMax() * vCount] = vY.image();
				}
				else
				{
					vY = CComplex(1.0 / (TwoTuned.R1 + TwoTuned.ConstDev->ddR1), vw * (TwoTuned.C2 + TwoTuned.ConstDev->ddC2) - 1.0 / vw / (TwoTuned.L2 + TwoTuned.ConstDev->ddL2));
			...
			case DCF_Triple:
				...
				if (ThreeTuned.R1 == 0)
				{
					...
					vZ += vY.inverse() + CComplex(0, vw * (ThreeTuned.L1 + ThreeTuned.ConstDev->ddL1) - 1.0 / vw / (ThreeTuned.C1 + ThreeTuned.ConstDev->ddC1));
					vY = vZ.inverse();
					ThreeTuned.ConstDevYg[vhOrder - 1 + hMax() * vCount] = vY.real();
					ThreeTuned.ConstDevYb[vhOrder - 1 + hMax() * vCount] = vY.image();
				}
				else
				{
					vY = CComplex(1.0 / (ThreeTuned.R2 + ThreeTuned.ConstDev->ddR2), vw * (ThreeTuned.C3 + ThreeTuned.ConstDev->ddC3) - 1.0 / vw / (ThreeTuned.L2 + ThreeTuned.ConstDev->ddL3));
	...
	for (int vCount = 0; vCount < RandTimes; vCount++)
		for (int vhOrder = 1; vhOrder <= hMax(); vhOrder++)
		{
			...
			case DCF_Single:
				...
				OneTuned.RandDevYg[vhOrder - 1 + hMax() * vCount] = vY.real();
				OneTuned.RandDevYb[vhOrder - 1 + hMax() * vCount] = vY.image();
				break;
			case DCF_Double:
				vw = PI * 2 * (Frequency)* vhOrder;
				if (TwoTuned.R1 == 0)
				{
					...
					vY = vZ.inverse();
					TwoTuned.RandDevYg[vhOrder - 1 + hMax() * vCount] = vY.real();
					TwoTuned.RandDevYb[vhOrder - 1 + hMax() * vCount] = vY.image();
				}
				else
				{
			...
			case DCF_Triple:
				...
				if (ThreeTuned.R1 == 0)
				{
					...
					vY = vZ.inverse();
					ThreeTuned.RandDevYg[vhOrder - 1 + hMax() * vCount] = vY.real();
					ThreeTuned.RandDevYb[vhOrder - 1 + hMax() * vCount] = vY.image();
				}
				else
				{
...
{
	Yg = OneTuned.ConstDevYg + vCount * hMax();
	Yb = OneTuned.ConstDevYb + vCount * hMax();
}

void CxbDevDCF::selectRandDevY(int vCount)
{
	Yg = OneTuned.RandDevYg + vCount * hMax();
	Yb = OneTuned.RandDevYb + vCount * hMax();
}
